...
def amass_to_pose(src_path, save_path):
    """
    Convert AMASS data to pose data.

    Runs the SMPL body model to get the joint positions. Swaps the Y/Z axis so that the human stands on the XZ plane instead of the XY plane.
    Saves the poses, trnaslations, betas, and gender as an npy file.

    Input:
        src_path: path to the AMASS data
            - has the following data structure:s
                - trans: (frame_num, 3)
                - gender: ex: array('female', dtype='<U6')
                - mocap_framerate: ex: array(120.)
                - betas: (16,)
                - dmpls: (frame_num, 8)
                - poses: (frame_num, 156)
            - Example:
                - src_path: './amass_data/HumanML3D/000000.npz'
        save_path: path to save the pose data
            - Example:
                - save_path: './pose_data/000000.npy'

    Output:
        fps: framerate of the pose data
    """
